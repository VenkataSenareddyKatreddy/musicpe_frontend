...
const Playlist = () => {
  ...
  const deletePlaylist = async () => {
    ...
    const { data, status } = await axios.delete(
      `https://musicpe-backend-vndg.onrender.com/api/v1/playlist/delete/${id}`,
      { headers }
    );
    if (status === 200) {
      setLoading(false);
  ...
  return loading || playList === null ? (
    ...
    <div className="bg-[conic-gradient(at_bottom_left,_var(--tw-gradient-stops))] from-gray-900 via-gray-100 to-gray-900 text-white p-5 min-h-screen space-y-5 flex flex-col lg:items-center">
